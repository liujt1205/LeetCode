class Solution:
                ...
                if grid[i][j] == 1:
                    ...
                    if discovery_time[i][j] == -1:  # If not yet visited
                        ...
                        self._find_articulation_points(
                            grid, i, j, discovery_time, lowest_reachable, parent_cell,
                            ap_info)
                        island_count += 1
    ...
    def _find_articulation_points(self, grid, row, col, discovery_time, lowest_reachable, parent_cell, ap_info):
        rows, cols = len(grid), len(grid[0])
        ...
        for direction in self.DIRECTIONS:
            ...
            if self._is_valid_land_cell(grid, new_row, new_col):
                if discovery_time[new_row][new_col] == -1:
                    ...
                    self._find_articulation_points(grid, new_row, new_col, discovery_time, lowest_reachable, parent_cell, ap_info)
